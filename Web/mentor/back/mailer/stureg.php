<?php

use PHPMailer\PHPMailer\PHPMailer;

    function sendOTP($s_email,$username)
    {
        require_once "PHPMailer/PHPMailer.php";
        require_once "PHPMailer/SMTP.php";
        require_once "PHPMailer/Exception.php";

        $mail = new PHPMailer();

        //SMTP Settings
        $mail->isSMTP();
        $mail->Host = "smtp.gmail.com";
        $mail->SMTPAuth = true;
        $mail->Username = "contact.zocarro@gmail.com";
        $mail->Password = 'zocarroadmin@123';
        $mail->Port = 465; //587
        $mail->SMTPSecure = "ssl"; //tls

        //Email Settings
        $mail->isHTML(true);
        $mail->setFrom('contact.zocarro@gmail.com','ZoCarro');
        $mail->addAddress($s_email);
        $mail->Subject = 'ZoCarro UserName For Student Request';
        $mail->Body = '<div style="margin:2%2%2%2%; border-style: dashed; border-color: orangered;background-color: whitesmoke;">
                                <h1 style="font-family:Georgia; text-align: center; color:#03a9f4"><u>ZoCarro</u></h1>
                                <div style="margin:4%4%4%4%">
                                    <p style="color: orangered;">Your Student Registration Request Is Approved And Your Username & Password Is ,</p>
                                    <p style="color: orangered;font-size: 18pt;">Username : '.$username.'</p>
                                    <p style="color: orangered;font-size: 18pt;">Password : abc@123</p>
                                </div>
                        </div>';
        $result = $mail->send();
        return $result;
    }
?>
