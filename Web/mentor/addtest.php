...
if(isset($_GET['testdata']) && isset($_GET['subdata']) && isset($_GET['chdata']) && isset($_GET['mentordata']))
{
	$v_id = mysqli_real_escape_string($con,$_GET['testdata']);
	$v_id = base64_decode($v_id);
	
	$sub_id = mysqli_real_escape_string($con,$_GET['subdata']);
	$sub_id = base64_decode($sub_id);
	
	$c_id = mysqli_real_escape_string($con,$_GET['mentordata']);
	$c_id = base64_decode($c_id);
	
	$ch_id = mysqli_real_escape_string($con,$_GET['chdata']);
	$ch_id = base64_decode($ch_id);
	
	$cmd3="select * from creator_video where v_id='$v_id'";
