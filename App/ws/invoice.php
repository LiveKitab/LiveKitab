...
// $decoded = base64_decode($bill_id);
    ...
    $id = mysqli_real_escape_string($conn,$_GET['bill_id']);
    // $id = base64_decode($id);
    ...
    $run=mysqli_query($conn,$sel) or die(mysqli_error($conn));
    while($row=mysqli_fetch_array($run))
    {
     ...
     $run1=mysqli_query($conn,$sel1) or die(mysqli_error($conn));
...
$return=mysqli_query($conn,$dis) or die(mysqli_error($conn));
