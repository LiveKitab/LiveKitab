	...
	/*	$c_id  = "KAU1";
		...
		if($count==0)
		{
			...
			while($row12=mysqli_fetch_assoc($result12)) 
			{
				...
				$des = $row12['v_des'];
				 $temp['v_des'] =  htmlspecialchars($des); 
				 $link  = $row12['v_link'];
				...
				$temp['v_des']=$row12['v_des'];
				 ...
				 $cmd5 = "SELECT id FROM video_wishlist WHERE user_id = '$user_id' AND vid ='$v_id'";
		...
		else
		{
			...
			while($row=mysqli_fetch_assoc($result))
			{
				...
				*/
				...
				$dis =$row['v_des'];
				$temp['v_des'] = htmlspecialchars($dis);
				...
				$temp['v_des']=$row['v_des'];
				 ...
				 $cmd6 = "SELECT id FROM video_wishlist WHERE user_id = '$user_id' AND vid ='$v_id1'";
				//  echo $cmd6;
                  $run6 = mysqli_query($conn,$cmd6) or die(mysqli_error($conn));
